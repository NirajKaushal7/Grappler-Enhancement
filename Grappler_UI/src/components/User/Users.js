...
import {addLogo, deleteUser, fetchUser } from "../../slices/userSlice"; // Import relevant Redux actions and slices
import { Link, NavLink, useNavigate } from "react-router-dom";
import { FaPlus } from "react-icons/fa";
import "./../../css/company.css"; // You can create a separate CSS file for the UserComponent
...
import { addImage,getAllUsers } from "../../api/UserApi"; // You'll need to adjust API calls and functions
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        dispatch(fetchUser(response.data)); // Use the appropriate Redux action
      } catch (error) {
  ...
  const handleUpload = async () => {
    if (userId,selectedFile) {
      ...
      if (selectedFile.size <= 1048576) {
        try {
          const response = await addImage(userId, selectedFile);
          console.log(response.data);
          const logoFileName = response.data.profile;
          dispatch(addLogo({ userId, logoFileName }));
          navigate(`/user`);
        } catch (error) {
    ...
    <div>
      <div className="container">
        ...
        <div className="right-aligned-div">
          <NavLink to = "/addUser" className="menu-bars">
            <IoIosAddCircle />
          </NavLink>
        </div>
              ...
              <tr key={user.id}>
                <td>
                  <div
                    ...
                    onClick={() => handleLogoClick(user.id)}
                  >
                    <img

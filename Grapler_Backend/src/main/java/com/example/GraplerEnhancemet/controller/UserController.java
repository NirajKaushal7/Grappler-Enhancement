...
public class UserController {
	...
	public ResponseEntity<ApiResponse<List<?>>> getAllUsersWithRoleByCompanyId(@PathVariable Long companyId) {
		try {
			List<UserWithRoleDTO> users = userService.getAllUsersAndRolesByCompanyId(companyId);
			if (!users.isEmpty()) {
				logger.info("All users with roles successfully retrieved");
				return ResponseEntity.ok(new ApiResponse<>(true, users, "All users retrieved successfully"));
			} else {
				logger.warn("No users available.");
				return ResponseEntity.status(HttpStatus.NOT_FOUND).body(new ApiResponse<>(false, null, "No users available"));

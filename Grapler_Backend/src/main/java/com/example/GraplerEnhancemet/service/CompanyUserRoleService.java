...
public class CompanyUserRoleService {
    ...
    public CompanyUserRole addCompanyUserRole(Long companyId, Long userId, CompanyUserRole companyUserRole){
        ...
        if(company == null) {
            logger.warn("Company Not Found on this Id : "+companyId);
            throw  new NotFoundException("Company Not Found on this Id : "+companyId);
        }
        User user = userService.getUserById(userId);
        if(user == null) {
            logger.warn("User Not Found on this Id : "+userId);
        ...
        if(role == null) {
            ...
            logger.warn("User Already assigned with a Role : "+role);
            throw new DuplicateDataException("User Already assigned with a Role : "+role);
        }
    }
    public CompanyUserRole updateCompanyUserRoleByC_IdAndU_Id(Long companyId, Long userId, CompanyUserRole companyUserRole){
        Company company = companyService.getCompanyById(companyId);
        if(company == null) {
            logger.warn("Company Not Found on this Id : "+companyId);
            throw  new NotFoundException("Company Not Found on this Id : "+companyId);
        ...
        CompanyUserRole existingCompanyUserRole = companyUserRoleRepository.findByCompany_IdAndUser_Id(companyId,userId);
        if(existingCompanyUserRole != null) {
            if(companyUserRole.getRole() != null) {
                existingCompanyUserRole.setRole(companyUserRole.getRole());
            }
            ...
            logger.warn("User role can't be Updated : This User does not contain any role in this Company");
            throw new NotFoundException("User role can't be Updated : This User does not contain any role in this Company");
        }
    }

...
public class UserService {
    ...
    public List<UserDTO> getAllUsersByCompanyId(Long companyId) {
        List<User> users = companyUserRoleRepository.findAllUsersByCompanyId(companyId);
        logger.info("Retrieved all users successfully.");
        return users.stream()
                .map(user -> modelMapper.map(user, UserDTO.class))
                .collect(Collectors.toList());
    }
    public List<UserWithRoleDTO> getAllUsersAndRolesByCompanyId(Long companyId) {
        List<UserWithRoleDTO> users = companyUserRoleRepository.findUsersAndRolesByCompanyId(companyId);
        logger.info("Retrieved all users with roles successfully.");
        return users;
    }
